# How to Install Scroll network to Brownie

### How to setup local brownie network config to support Scroll
- Find where the file is installed, usually its `/Users/user/.brownie/network-config.yaml`
...
```
  - name: Ganache-CLI (Scroll-Alpha-Testnet Fork)
    id: scroll-alpha-fork
    cmd: ganache-cli
    host: http://127.0.0.1
    timeout: 120
    cmd_settings:
...
### If it's easier, here's the whole file:
```
live:
  - name: Ethereum
    networks:
